...
class MultiPrototypeTransductiveInference(nn.Module):
    ...
    def getMutiplePrototypes(self, feat, k):
        ...
        if ratio < 1:
            ...
            distances = self.square_distance(feat[None,...], farthest_seeds[None,...])
            distances = distances.squeeze() # (n_points, n_prototypes)
    ...
    def getForegroundPrototypes(self, feats, masks, k=100):
        ...
        for i in range(self.n_way):
            ...
            class_labels[:, i+1] = 1 #第0列为背景标签，因此前景标签从第1列开始
    ...
    def calculateLocalConstrainedAffinity(self, node_feat, k=200, method='gaussian'):
        ...
        if method == 'cosine':
            ...
            node_feat = node_feat.repeat(1, k, 1)
            knn_similarity = F.cosine_similarity(node_feat, knn_feat, dim=2)
            knn_similarity = knn_similarity[:, 0]
        elif method == 'gaussian':
            # dist = F.pairwise_distance(node_feat[:,:,None], knn_feat.transpose(1,2), p=2)
            dist = self.square_distance(node_feat.unsqueeze(1), knn_feat).squeeze()
            knn_similarity = torch.exp(-0.5*(dist/self.sigma)**2)
        else:
            raise NotImplementedError('Error! Distance computation method (%s) is unknown!' %method)

        A = torch.zeros(self.num_nodes, self.num_nodes).cuda()
    ...
    def label_propagate(self, A, Y, alpha=0.99):
        ...
        D_sqrt_inv = torch.diag_embed(D_sqrt_inv).cuda() # 生成对角矩阵
        S = D_sqrt_inv @ A @ D_sqrt_inv # @可以用于计算矩阵乘法
